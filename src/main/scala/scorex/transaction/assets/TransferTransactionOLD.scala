...
case class TransferTransactionOLD private(assetId: Option[AssetId],
  ...
  override val transactionType: TransactionType.Value = TransactionType.TransferTransactionOLD
...
object TransferTransactionOLD {

  val MaxAttachmentSize = 140
  val MaxAttachmentStringSize = base58Length(MaxAttachmentSize)


  def parseTail(bytes: Array[Byte]): Try[TransferTransactionOLD] = Try {
    import EllipticCurveImpl._

    val signature = ByteStr(bytes.slice(0, SignatureLength))
    val txId = bytes(SignatureLength)
    require(txId == TransactionType.TransferTransactionOLD.id.toByte, s"Signed tx id is not match")
    ...
    (for {
      ...
      tt <- TransferTransactionOLD.create(assetIdOpt.map(ByteStr(_)), sender, recipient, amount, timestamp, feeAssetIdOpt.map(ByteStr(_)), feeAmount, attachment, signature)
  ...
  def create(assetId: Option[AssetId],
             ...
             feeAmount: Long,
             attachment: Array[Byte],
             signature: ByteStr): Either[ValidationError, TransferTransactionOLD] = {
    if (attachment.length > TransferTransactionOLD.MaxAttachmentSize) {
      ...
      Right(TransferTransactionOLD(assetId, sender, recipient, amount, timestamp, feeAssetId, feeAmount, attachment, signature))
    }
  }
  ...
  def create(assetId: Option[AssetId],
             ...
             feeAssetId: Option[AssetId],
             feeAmount: Long,
             attachment: Array[Byte]): Either[ValidationError, TransferTransactionOLD] = {

...
class BlockchainUpdaterBadReferencesTest extends PropSpec with PropertyChecks with
  ...
  val preconditionsAndPayments: Gen[(GenesisTransaction, TransferTransactionOLD, TransferTransactionOLD, TransferTransactionOLD)] = for {
    master <- accountGen
    recipient <- accountGen
    ts <- positiveIntGen
    genesis: GenesisTransaction = GenesisTransaction.create(master, ENOUGH_AMT, ts).right.get
    payment: TransferTransactionOLD <- wavesTransferGeneratorP(master, recipient)
    payment2: TransferTransactionOLD <- wavesTransferGeneratorP(master, recipient)
    payment3: TransferTransactionOLD <- wavesTransferGeneratorP(master, recipient)

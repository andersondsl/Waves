...
class BlockchainUpdaterGeneratorFeeNextBlockOrMicroBlockTest extends PropSpec with PropertyChecks with DomainScenarioDrivenPropertyCheck with Matchers with TransactionGen {
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    somePayment: TransferTransactionOLD = createWavesTransfer(sender, recipient, 1, 10, ts+1).right.get
    // generator has enough balance for this transaction if gets fee for block before applying it
    generatorPaymentOnFee: TransferTransactionOLD = createWavesTransfer(defaultSigner, recipient, 11, 1, ts + 2).right.get
    someOtherPayment: TransferTransactionOLD = createWavesTransfer(sender, recipient, 1, 1, ts + 3).right.get

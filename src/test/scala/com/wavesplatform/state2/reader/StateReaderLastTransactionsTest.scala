...
class StateReaderLastTransactionsTest extends PropSpec
  ...
  val preconditionsAndPayment: Gen[(Seq[Transaction], TransferTransactionOLD)] = for {
    master <- accountGen
    recipient <- accountGen
    ts <- timestampGen
    genesis: GenesisTransaction = GenesisTransaction.create(master, ENOUGH_AMT, ts).right.get
    transfer1: TransferTransactionOLD <- wavesTransferGeneratorP(ts + 1, master, recipient)
    transfer2: TransferTransactionOLD <- wavesTransferGeneratorP(ts + 2, master, recipient)
    preconditions: Seq[Transaction] = Seq(genesis, transfer1, transfer2)

    transfer3: TransferTransactionOLD <- wavesTransferGeneratorP(ts + 3, master, recipient)
